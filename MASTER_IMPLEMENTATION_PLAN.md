...
- [ ] **Vessel-detail integration pass (step 10):** not a new module — adds Certificates/Crew/Insurance/Manuals/Drawings/Particulars sections to the existing vessel detail page (`src/app/dashboard/vessels/[id]/page.tsx`) as one consolidated pass, per `PROJECT_PLAN.md` §1's explicit note that this is deferred until Crew and Insurance both ship rather than done per-module. **Tool:** Cursor-led (assembling existing pieces into tabs/sections on one page is exactly the live-layout work Cursor is suited for); Claude Code only if a new cross-module query helper turns out to be needed.
