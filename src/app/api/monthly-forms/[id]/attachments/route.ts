...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const detail = await getMonthlyFormById(access, id);
  if (!detail) {
    return NextResponse.json({ error: "Form not found" }, { status: 404 });
  }
  const data = await listMonthlyFormAttachments(access, id);
  return NextResponse.json({ data });
}

/**
 * POST multipart upload — same as submit when adding another file to an
 * already-submitted form (or first submit if remarks omitted).
 */
