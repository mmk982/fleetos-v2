...
import { reminderUpdateSchema } from "@/modules/reminders/validation";
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const row = await getReminderById(toAccessContext(session), id);
  if (!row) {
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    const data = await updateReminder(access, id, parsed.data);
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  const { id } = await context.params;
  try {
    await deleteReminder(toAccessContext(session), id);
