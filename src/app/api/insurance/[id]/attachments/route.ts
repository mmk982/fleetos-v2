...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const detail = await getInsurancePolicyById(access, id);
  if (!detail) {
  ...
  const data = await listInsuranceAttachments(access, id);
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const data = await uploadInsuranceAttachment(access, id, {
      name: file.name,
