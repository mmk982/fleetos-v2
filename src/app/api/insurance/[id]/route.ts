...
import { insuranceUpdateSchema } from "@/modules/insurance/validation";
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    const data = await updateInsurancePolicy(access, id, parsed.data);
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  const { id } = await context.params;
  try {
    await deleteInsurancePolicy(toAccessContext(session), id);
